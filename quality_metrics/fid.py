...
class FrechetInceptionDistance(Metric):
    ...
    def __init__(
        ...
        input_img_size: Tuple = (3, 299, 299),
        ...
        **kwargs: Any,
        ...
        if isinstance(feature, int):
            ...
            dtype = torch.uint8 if img_type == "byte" else torch.float32
            dummy_image = torch.randint(0, 255, (1, *input_img_size), dtype=dtype)
            dummy_image = dummy_image.to(device) if device is not None else dummy_image
    ...
    def update(self, imgs: Tensor, real: bool) -> None:
        ...
        imgs = (imgs * 255) if self.normalize else imgs
        imgs = imgs.byte() if self.img_type == "byte" else imgs.float()
