...
class VectorQuantize(nn.Module):
    def __init__(
        ...
        self.project_out = (
            nn.Linear(codebook_input_dim, out_dim)
    ...
    def forward(
        ...
        if self.training:
            ...
            maybe_detach = (
                ...
                or freeze_codebook
                or self.commitment_weight > 0
                else identity
            )

            # if not self.learnable_codebook or freeze_codebook or self.commitment_weight > 0:
            #     debug("Using quantize for commitment loss")
            #     debug(self.learnable_codebook, freeze_codebook, self.commitment_weight)

            commit_quantize = maybe_detach(quantize)

            # compute q-loss
            q_loss = (
                F.mse_loss(quantize, x.detach()) if self.learnable_codebook else 0.0
            )
            # debug(f"Q-loss: {q_loss.item()}")
        ...
        if self.training:
            ...
            if self.commitment_weight > 0:
                ...
                # debug(f"Commitment loss: {commit_loss.item()}")
